...
class State:
    def __init__(self, device):
        ...
        self.callbacky = FnVoid_VoidP_DataP(self.data_handlery)
        self.callbackz = FnVoid_VoidP_DataP(self.data_handlerz)
    ...
    def data_handlery(self, ctx, data):
        #print("%s -> %s" % (self.device.address, parse_value(data)))
        # print("%s" % (parse_value(data)))
        z=parse_value(data)
        print("y = ", z)
...
for s in states:
    ...
    signal_acc_y = libmetawear.mbl_mw_datasignal_get_component(signal_acc, Const.ACC_ACCEL_Y_AXIS_INDEX)
    libmetawear.mbl_mw_datasignal_subscribe(signal_acc_y, None, s.callbacky)
    signal_acc_z = libmetawear.mbl_mw_datasignal_get_component(signal_acc, Const.ACC_ACCEL_Z_AXIS_INDEX)
